...
if(isset($_POST['submit_student'])){
    ...
    if(!isset($error)){
        ...
        if($id){ echo "<script>alert('Student added');window.location.href='students.php';</script>"; exit; } else { $error = "Unable to add"; }
    }
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <script>
        function sameAsCurrent(cb){ if(cb.checked){ ['house_street','barangay','city','province','zip'].forEach(function(f){ document.getElementById('perm_'+f).value = document.getElementById('curr_'+f).value; }); } }
    </script>
</head>
<body>
<div class="container">
    ...
    <form method="post">
        ...
        <div class="section">
            <div class="grid">
                ...
                <div><label>Sex</label><select name="sex" required><option>Male</option><option>Female</option></select></div>
            </div>
        </div>
        <div class="section">
            <div class="grid">
        ...
        <div class="section">
            <button name="submit_student">Add Student</button>
        </div>
    </form>
