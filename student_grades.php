...
if($db_available && $crud) {
    try {
        ...
        if($sy){
            ...
            foreach($all as $e){ 
                if($e['school_year']===$sy && ($sem==='' || ($e['semester']??'')===$sem)) {
                    $gs = $pdo->prepare('SELECT q1,q2,q3,q4,grade FROM grades WHERE enrollment_id = :id');
                    $gs->execute([':id'=>$e['id']]);
                    $g = $gs->fetch();
                    $e['q1'] = $g['q1'] ?? null;
                    $e['q2'] = $g['q2'] ?? null;
        ...
        $available_years = array_values(array_unique(array_map(function($y){ return trim((string)$y); }, $available_years)));
    } catch(Exception $e) {
        $db_available = false;
    }
}
                    ...
                    <form method="get" class="d-flex gap-4 flex-wrap" style="align-items: flex-end;">
                        <div class="form-group" style="margin-bottom: 0; min-width: 200px;">
                            ...
                            <select name="sy" class="form-control" required>
                                ...
                                <?php endforeach; ?>
                                <?php 
                                $default_sy = date('Y').'-'.(date('Y')+1);
                                if (empty($available_years)) { 
                                    $val = $sy ?: $default_sy; 
                                ?>
                                <option value="<?php echo htmlspecialchars($val, ENT_QUOTES, 'UTF-8'); ?>" <?php echo $sy ? 'selected' : ''; ?>><?php echo htmlspecialchars($val, ENT_QUOTES, 'UTF-8'); ?></option>
                                <?php } elseif($sy && !in_array($sy, $available_years)) { ?>
                                <option value="<?php echo htmlspecialchars($sy, ENT_QUOTES, 'UTF-8'); ?>" selected><?php echo htmlspecialchars($sy, ENT_QUOTES, 'UTF-8'); ?></option>
                                <?php } ?>
                            </select>
                        </div>
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php $q1 = $e['q1'] ?? null; ?>
                                        <?php echo $q1 !== null ? htmlspecialchars($q1, ENT_QUOTES, 'UTF-8') : '<span class="text-muted">-</span>'; ?>
                                    </td>
                                    <td>
                                        <?php $q2 = $e['q2'] ?? null; ?>
                                        <?php echo $q2 !== null ? htmlspecialchars($q2, ENT_QUOTES, 'UTF-8') : '<span class="text-muted">-</span>'; ?>
                                    </td>
                                    <td>
                                        <?php $q3 = $e['q3'] ?? null; ?>
                                        <?php echo $q3 !== null ? htmlspecialchars($q3, ENT_QUOTES, 'UTF-8') : '<span class="text-muted">-</span>'; ?>
                                    </td>
                                    <td>
                                        <?php $q4 = $e['q4'] ?? null; ?>
                                        <?php echo $q4 !== null ? htmlspecialchars($q4, ENT_QUOTES, 'UTF-8') : '<span class="text-muted">-</span>'; ?>
                                    </td>
                                    <td>
                                        <?php if($e['grade'] !== null): ?>
