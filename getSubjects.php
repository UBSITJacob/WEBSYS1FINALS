...
$sort = $_GET['sort'] ?? 'name';
$dir = $_GET['dir'] ?? 'ASC';
$rows = $pdo->getSubjects($q,$page,$limit,$sort,$dir);
$total = $pdo->countSubjects($q);
$totalPages = max(1, ceil($total/$limit));
?>
<table class="table">
    <thead>
        <tr>
            <th onclick="setSort('code')" class="cursor-pointer">Code</th>
            <th onclick="setSort('name')" class="cursor-pointer">Name</th>
            <th>Department</th>
            <th onclick="setSort('created_at')" class="cursor-pointer">Grade Level</th>
            <th>Strand</th>
            <th>Semester</th>
            <th>Description</th>
            <th>Actions</th>
    ...
    <tbody>
        <?php if(!$rows): ?>
        <tr>
            <td colspan="8" class="text-center text-muted p-6">No subjects found</td>
        </tr>
        <?php else: ?>
        <?php foreach($rows as $r): ?>
        <tr>
            <td class="font-medium"><?php echo htmlspecialchars($r['code'] ?? '',ENT_QUOTES,'UTF-8'); ?></td>
            ...
            <td>
                <div class="table-actions">
                    <button onclick="editSubject(this)" class="btn btn-sm btn-accent" title="Edit Subject"
                            ...
                            data-dept="<?php echo htmlspecialchars($r['department'],ENT_QUOTES,'UTF-8'); ?>"
                            data-grade="<?php echo htmlspecialchars($r['grade_level'],ENT_QUOTES,'UTF-8'); ?>"
                            data-strand="<?php echo htmlspecialchars($r['strand'] ?? '',ENT_QUOTES,'UTF-8'); ?>"
                    ...
                    <button onclick="deleteSubject(<?php echo (int)$r['id']; ?>)" class="btn btn-sm btn-danger" title="Delete Subject">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
<div class="d-flex justify-between align-center p-4 border-top">
    ...
    <div class="pagination">
        <button class="pagination-item <?php echo $page <= 1 ? 'disabled' : ''; ?>" onclick="<?php echo $page > 1 ? 'page--; loadSubjects();' : ''; ?>">
        ...
        <button class="pagination-item <?php echo $page >= $totalPages ? 'disabled' : ''; ?>" onclick="<?php echo $page < $totalPages ? 'page++; loadSubjects();' : ''; ?>">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
