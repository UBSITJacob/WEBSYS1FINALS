...
function saveAllGrades() {
    ...
    rows.forEach(row => {
        ...
        const q1 = row.querySelector('[data-period="q1"]').value || '';
        const q2 = row.querySelector('[data-period="q2"]').value || '';
        const q3 = row.querySelector('[data-period="q3"]').value || '';
        const q4 = row.querySelector('[data-period="q4"]').value || '';
        const finalGrade = row.querySelector('.final-grade').value || '';
        if (finalGrade) {
            grades.push({ enrollment_id: eid, grade: finalGrade, q1, q2, q3, q4 });
        }
    });
    ...
    grades.forEach(g => {
        const body = 'enrollment_id=' + g.enrollment_id +
                     '&grade=' + encodeURIComponent(g.grade) +
                     '&q1=' + encodeURIComponent(g.q1) +
                     '&q2=' + encodeURIComponent(g.q2) +
                     '&q3=' + encodeURIComponent(g.q3) +
                     '&q4=' + encodeURIComponent(g.q4);
        fetch('saveGrade.php', {
            method: 'POST',
