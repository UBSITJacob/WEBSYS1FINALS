...
class pdoCRUD{
    ...
    public function getApplicants($q,$page,$limit,$sort='created_at',$dir='DESC',$status=''){
        $offset = ($page-1)*$limit;
        $q1 = "%".$q."%";
        $allowed = ['created_at','family_name','lrn','grade_level'];
        if(!in_array($sort,$allowed)) $sort = 'created_at';
        $dir = strtoupper($dir)==='ASC' ? 'ASC' : 'DESC';
        $where = "(CONCAT(family_name,' ',first_name) LIKE :q1 OR lrn LIKE :q2)";
        if($status !== ''){ $where .= " AND status = :status"; }
        $stmt = $this->pdo->prepare("SELECT * FROM applicants WHERE $where ORDER BY $sort $dir LIMIT :limit OFFSET :offset");
        $stmt->bindValue(':q1',$q1, PDO::PARAM_STR);
        $stmt->bindValue(':q2',$q1, PDO::PARAM_STR);
    ...
    public function countApplicants($q,$status=''){
        $q1 = "%".$q."%";
        $where = "(CONCAT(family_name,' ',first_name) LIKE :q1 OR lrn LIKE :q2)";
        if($status !== ''){ $where .= " AND status = :status"; }
        $c = $this->pdo->prepare("SELECT COUNT(*) c FROM applicants WHERE $where");
        $params = [':q1'=>$q1, ':q2'=>$q1];
    ...
    public function approveApplicant($id,$adminId=null){
        ...
        $upd = $this->pdo->prepare("UPDATE applicants SET status='approved', status_changed_at=NOW(), soft_delete_expires_at=DATE_ADD(NOW(), INTERVAL 7 DAY), last_action_by=:aid WHERE id=:id");
        $upd->execute([':id'=>$id, ':aid'=>$adminId]);
        $hist = $this->pdo->prepare("INSERT INTO applicant_history(applicant_id,action,action_by,purge_after,lrn,email,full_name,payload) VALUES(:aid,'approved',:by,DATE_ADD(NOW(), INTERVAL 7 DAY),:lrn,:email,:name,:payload)");
        $payload = json_encode($a);
        $hist->execute([':aid'=>$id, ':by'=>$adminId, ':lrn'=>$a['lrn'], ':email'=>$a['email'], ':name'=>$a['family_name'].', '.$a['first_name'], ':payload'=>$payload]);
        $this->pdo->commit();
        return true;
    ...
    public function getStudentByLRN($lrn){
        $s = $this->pdo->prepare("SELECT * FROM students WHERE lrn = :lrn LIMIT 1");
        $s->execute([':lrn'=>$lrn]);
        return $s->fetch();
    }

    public function declineApplicant($id,$adminId=null){
        $this->pdo->beginTransaction();
        try{
        $a = $this->getApplicantById($id);
        if(!$a){ $this->pdo->rollBack(); return false; }
        $d = $this->pdo->prepare("UPDATE applicants SET status='declined', status_changed_at=NOW(), soft_delete_expires_at=DATE_ADD(NOW(), INTERVAL 7 DAY), last_action_by=:aid WHERE id=:id");
        $d->execute([':id'=>$id, ':aid'=>$adminId]);
        $hist = $this->pdo->prepare("INSERT INTO applicant_history(applicant_id,action,action_by,purge_after,lrn,email,full_name,payload) VALUES(:aid,'declined',:by,DATE_ADD(NOW(), INTERVAL 7 DAY),:lrn,:email,:name,:payload)");
        $payload = json_encode($a);
        $hist->execute([':aid'=>$id, ':by'=>$adminId, ':lrn'=>$a['lrn'], ':email'=>$a['email'], ':name'=>$a['family_name'].', '.$a['first_name'], ':payload'=>$payload]);
        $this->pdo->commit();
        return true;
    ...
    public function purgeDecidedApplicants(){
        $p = $this->pdo->prepare("DELETE FROM applicants WHERE status!='pending' AND soft_delete_expires_at IS NOT NULL AND soft_delete_expires_at <= NOW()");
        return $p->execute();
    }

    // Sections
    public function getSections($q,$page,$limit,$sort='grade_level',$dir='ASC',$grade=null,$dept=null){
        $offset = ($page-1)*$limit;
        $qLike = "%".$q."%";
        ...
        $where = "name LIKE :q";
        if($grade !== null && $grade !== ''){ $where .= " AND grade_level = :grade"; }
        if($dept !== null && $dept !== ''){ $where .= " AND department = :dept"; }
        $s = $this->pdo->prepare("SELECT * FROM sections WHERE $where ORDER BY $sort $dir, name ASC LIMIT :limit OFFSET :offset");
        $s->bindValue(':q',$qLike,PDO::PARAM_STR);
        if($grade !== null && $grade !== ''){ $s->bindValue(':grade',$grade,PDO::PARAM_STR); }
        if($dept !== null && $dept !== ''){ $s->bindValue(':dept',$dept,PDO::PARAM_STR); }
    ...
    public function countSections($q,$grade=null,$dept=null){
        $qLike = "%".$q."%";
        $where = "name LIKE :q";
        $params = [':q'=>$qLike];
        if($grade !== null && $grade !== ''){ $where .= " AND grade_level = :grade"; $params[':grade'] = $grade; }
        if($dept !== null && $dept !== ''){ $where .= " AND department = :dept"; $params[':dept'] = $dept; }
        $s = $this->pdo->prepare("SELECT COUNT(*) c FROM sections WHERE $where");
        $s->execute($params);
    ...
    public function getSectionMeta($id){
        $s = $this->pdo->prepare("SELECT department, grade_level, strand FROM sections WHERE id=:id");
        $s->execute([':id'=>$id]);
        return $s->fetch();
    }
    ...
    // Subjects
   public function getSubjects($q,$page,$limit,$sort='name',$dir='ASC'){
    $offset = ($page-1)*$limit;
    $qLike = "%".$q."%";
    $allowed = ['id','code','name','created_at','updated_at'];
    if(!in_array($sort,$allowed)) $sort = 'name';
    $dir = strtoupper($dir)==='DESC' ? 'DESC' : 'ASC';
    $sql = "SELECT id, code, name, department, grade_level, strand, semester, description, created_at, updated_at
                FROM subjects
    ...
    $s->bindValue(':q1',$qLike,PDO::PARAM_STR);
    $s->bindValue(':q2',$qLike,PDO::PARAM_STR);
    $s->bindValue(':limit',(int)$limit,PDO::PARAM_INT);
    $s->bindValue(':offset',(int)$offset,PDO::PARAM_INT);
    $s->execute();
    return $s->fetchAll();
    ...
    public function countSubjects($q){
        $qLike = "%".$q."%";
        $s = $this->pdo->prepare("SELECT COUNT(*) c FROM subjects WHERE name LIKE :q1 OR code LIKE :q2");
        $s->execute([':q1'=>$qLike, ':q2'=>$qLike]);
        return (int)$s->fetch()['c'];
    }

    public function addSubject($code,$name,$description,$department,$grade_level,$strand=null,$semester=null){
        $i = $this->pdo->prepare("INSERT INTO subjects(code,name,department,grade_level,strand,semester,description) VALUES(:code,:name,:department,:grade_level,:strand,:semester,:description)");
        $i->execute([
            ':code'=>$code?:null,
            ':name'=>$name,
    ...
    public function updateSubject($id,$code,$name,$description,$department,$grade_level,$strand=null,$semester=null){
        $u = $this->pdo->prepare("UPDATE subjects SET code=:code, name=:name, department=:department, grade_level=:grade_level, strand=:strand, semester=:semester, description=:description WHERE id=:id");
        return $u->execute([
            ':id'=>$id,
            ':code'=>$code?:null,
            ':name'=>$name,
    ...
    public function deleteSubject($id){
        try {
            $this->pdo->beginTransaction();
            $d = $this->pdo->prepare("DELETE FROM subjects WHERE id=:id");
            $ok = $d->execute([':id'=>$id]);
            if(!$ok){ $this->pdo->rollBack(); return false; }
            $this->pdo->commit();
            return true;
    ...
    public function getSubjectById($id){
        $s = $this->pdo->prepare("SELECT * FROM subjects WHERE id=:id");
        $s->execute([':id'=>$id]);
        return $s->fetch();
    }

    // Teachers
    public function getTeachers($q,$page,$limit,$sort='full_name',$dir='ASC',$active=null){
        $offset = ($page-1)*$limit;
        $qLike = "%".$q."%";
        $allowed = ['full_name','faculty_id','email','active'];
        if(!in_array($sort,$allowed)) $sort = 'full_name';
        $dir = strtoupper($dir)==='DESC' ? 'DESC' : 'ASC';
        $where = "(t.full_name LIKE :q1 OR t.faculty_id LIKE :q2)";
        if($active !== null && $active !== ''){ $where .= " AND t.active = :active"; }
        $sql = "SELECT t.*, s.name AS advisory_section_name, s.grade_level AS advisory_grade_level, s.department AS advisory_department
                FROM teachers t
                LEFT JOIN sections s ON s.id = t.advisory_section_id
                WHERE $where
                ORDER BY t.$sort $dir
                LIMIT :limit OFFSET :offset";
        $s = $this->pdo->prepare($sql);
        $s->bindValue(':q1',$qLike,PDO::PARAM_STR);
        $s->bindValue(':q2',$qLike,PDO::PARAM_STR);
        if($active !== null && $active !== ''){ $s->bindValue(':active',(int)$active,PDO::PARAM_INT); }
        $s->bindValue(':limit',(int)$limit,PDO::PARAM_INT);
        $s->bindValue(':offset',(int)$offset,PDO::PARAM_INT);
        $s->execute();
        return $s->fetchAll();
    ...
    // Admins
    public function getAdmins($q,$page,$limit,$sort='full_name',$dir='ASC'){
        $offset = ($page-1)*$limit;
        $qLike = "%".$q."%";
        $allowed = ['full_name','faculty_id','username','email','sex','created_at'];
        if(!in_array($sort,$allowed)) $sort = 'full_name';
        $dir = strtoupper($dir)==='DESC' ? 'DESC' : 'ASC';
        $sql = "SELECT * FROM admins WHERE (full_name LIKE :q1 OR faculty_id LIKE :q2 OR username LIKE :q3 OR email LIKE :q4) ORDER BY $sort $dir, full_name ASC LIMIT :limit OFFSET :offset";
        $s = $this->pdo->prepare($sql);
        $s->bindValue(':q1',$qLike,PDO::PARAM_STR);
        $s->bindValue(':q2',$qLike,PDO::PARAM_STR);
        $s->bindValue(':q3',$qLike,PDO::PARAM_STR);
        $s->bindValue(':q4',$qLike,PDO::PARAM_STR);
        $s->bindValue(':limit',(int)$limit,PDO::PARAM_INT);
        $s->bindValue(':offset',(int)$offset,PDO::PARAM_INT);
        $s->execute();
        return $s->fetchAll();
    ...
    public function countAdmins($q){
        $qLike = "%".$q."%";
        $c = $this->pdo->prepare("SELECT COUNT(*) c FROM admins WHERE (full_name LIKE :q1 OR faculty_id LIKE :q2 OR username LIKE :q3 OR email LIKE :q4)");
        $c->execute([':q1'=>$qLike, ':q2'=>$qLike, ':q3'=>$qLike, ':q4'=>$qLike]);
        return (int)$c->fetch()['c'];
    }

    public function addAdmin($faculty_id,$full_name,$username,$sex,$email){
        $this->pdo->beginTransaction();
        try{
            $i = $this->pdo->prepare("INSERT INTO admins(faculty_id,full_name,username,email,sex) VALUES(:fid,:name,:user,:email,:sex)");
            $i->execute([':fid'=>$faculty_id, ':name'=>$full_name, ':user'=>$username, ':email'=>$email, ':sex'=>$sex]);
            $aid = $this->pdo->lastInsertId();
            $acc = $this->pdo->prepare("INSERT INTO accounts(email,username,password_hash,role,first_login_required,person_type,person_id) VALUES(:email,:username,:ph,'admin',1,'admin',:pid)");
            $acc->execute([':email'=>$email, ':username'=>$username, ':ph'=>password_hash('1', PASSWORD_DEFAULT), ':pid'=>$aid]);
            $this->pdo->commit();
            return $aid;
        }catch(Exception $e){
            if($this->pdo->inTransaction()) $this->pdo->rollBack();
    ...
    public function updateAdmin($id,$full_name,$username,$sex,$email){
        $u = $this->pdo->prepare("UPDATE admins SET full_name=:name, username=:user, email=:email, sex=:sex WHERE id=:id");
        $ok = $u->execute([':id'=>$id, ':name'=>$full_name, ':user'=>$username, ':email'=>$email, ':sex'=>$sex]);
        $ua = $this->pdo->prepare("UPDATE accounts SET email=:email, username=:user WHERE person_type='admin' AND person_id=:id");
    ...
    public function deleteAdmin($id){
        try {
            $this->pdo->beginTransaction();
            $this->pdo->prepare("DELETE FROM accounts WHERE person_type='admin' AND person_id=:id")->execute([':id'=>$id]);
            $ok = $this->pdo->prepare("DELETE FROM admins WHERE id=:id")->execute([':id'=>$id]);
            if(!$ok){ $this->pdo->rollBack(); return false; }
            $this->pdo->commit();
            return true;
        } catch(Exception $e) {
    ...
    public function countTeachers($q,$active=null){
        $qLike = "%".$q."%";
        $where = "(full_name LIKE :q1 OR faculty_id LIKE :q2)";
        $params = [':q1'=>$qLike, ':q2'=>$qLike];
        if($active !== null && $active !== ''){ $where .= " AND active = :active"; $params[':active'] = (int)$active; }
        $s = $this->pdo->prepare("SELECT COUNT(*) c FROM teachers WHERE $where");
        $s->execute($params);
        return (int)$s->fetch()['c'];
    }
    ...
    public function setTeacherAdviser($teacher_id,$section_id){
        $this->pdo->beginTransaction();
        try{
            $chk = $this->pdo->prepare("SELECT id FROM sections WHERE id=:id");
            $chk->execute([':id'=>$section_id]);
            if(!$chk->fetch()){ $this->pdo->rollBack(); return false; }
            $u = $this->pdo->prepare("UPDATE teachers SET advisory_section_id=:sec WHERE id=:tid");
            $ok = $u->execute([':sec'=>$section_id, ':tid'=>$teacher_id]);
            if(!$ok){ $this->pdo->rollBack(); return false; }
            $this->pdo->commit();
            return true;
        }catch(Exception $e){ $this->pdo->rollBack(); return false; }
    ...
    // Students
    public function getStudents($q,$page,$limit,$sort='grade_level',$dir='ASC',$grade=null,$dept=null){
        $offset = ($page-1)*$limit;
        $qLike = "%".$q."%";
        $allowed = ['family_name','lrn','department','grade_level','strand'];
        if(!in_array($sort,$allowed)) $sort = 'grade_level';
        $dir = strtoupper($dir)==='DESC' ? 'DESC' : 'ASC';
        $where = "(CONCAT(s.family_name,' ',s.first_name) LIKE :q1 OR s.lrn LIKE :q2)";
        if($grade !== null && $grade !== ''){ $where .= " AND s.grade_level = :grade"; }
        if($dept !== null && $dept !== ''){ $where .= " AND s.department = :dept"; }
        $sql = "SELECT s.*, CASE WHEN a.id IS NULL THEN 0 ELSE 1 END AS has_account
                FROM students s
                LEFT JOIN accounts a ON a.person_type='student' AND a.person_id=s.id
                WHERE $where
                ORDER BY s.$sort $dir, s.family_name ASC
                LIMIT :limit OFFSET :offset";
        $s = $this->pdo->prepare($sql);
        $s->bindValue(':q1',$qLike,PDO::PARAM_STR);
        $s->bindValue(':q2',$qLike,PDO::PARAM_STR);
    ...
    public function countStudents($q,$grade=null,$dept=null){
        $qLike = "%".$q."%";
        $where = "(CONCAT(family_name,' ',first_name) LIKE :q1 OR lrn LIKE :q2)";
        $params = [':q1'=>$qLike, ':q2'=>$qLike];
        if($grade !== null && $grade !== ''){ $where .= " AND grade_level = :grade"; $params[':grade'] = $grade; }
        if($dept !== null && $dept !== ''){ $where .= " AND department = :dept"; $params[':dept'] = $dept; }
        $s = $this->pdo->prepare("SELECT COUNT(*) c FROM students WHERE $where");
        $s->execute($params);
        return (int)$s->fetch()['c'];
    }
    ...
    public function getAdvisoryStudentsForTeacher($teacher_id,$q,$page,$limit,$sort='family_name',$dir='ASC'){
        ...
        $dir = strtoupper($dir)==='DESC' ? 'DESC' : 'ASC';
        $s = $this->pdo->prepare("SELECT * FROM students WHERE advisory_section_id=:sec AND (CONCAT(family_name,' ',first_name) LIKE :q1 OR lrn LIKE :q2) ORDER BY $sort $dir LIMIT :limit OFFSET :offset");
    ...
    public function countAdvisoryStudentsForTeacher($teacher_id,$q){
        ...
        $sec = (int)$adv['advisory_section_id'];
        $qLike = "%".$q."%";
        $c = $this->pdo->prepare("SELECT COUNT(*) c FROM students WHERE advisory_section_id=:sec AND (CONCAT(family_name,' ',first_name) LIKE :q1 OR lrn LIKE :q2)");
        $c->execute([':sec'=>$sec, ':q1'=>$qLike, ':q2'=>$qLike]);
        return (int)$c->fetch()['c'];
    }
    ...
    public function setStudentPassword($student_id,$new_password){
        if(strlen($new_password) < 8) return false;
        $h = password_hash($new_password, PASSWORD_DEFAULT);
        $u = $this->pdo->prepare("UPDATE accounts SET password_hash=:h, first_login_required=0 WHERE person_type='student' AND person_id=:id");
        return $u->execute([':h'=>$h, ':id'=>$student_id]);
    }

    public function hasStudentAccount($student_id){
        $s = $this->pdo->prepare("SELECT id FROM accounts WHERE person_type='student' AND person_id=:id LIMIT 1");
    ...
    public function getSubjectLoads($q,$page,$limit,$sort='school_year',$dir='DESC'){
        ...
        $sql = "SELECT sl.*, t.full_name AS teacher_name, s.code AS subject_code, s.name AS subject_name, sec.name AS section_name
                ...
                WHERE t.full_name LIKE :q1 OR s.name LIKE :q2 OR sec.name LIKE :q3
                ORDER BY $sort $dir, sec.name ASC
                LIMIT :limit OFFSET :offset";
        $st = $this->pdo->prepare($sql);
        $st->bindValue(':q1',$qLike,PDO::PARAM_STR);
        $st->bindValue(':q2',$qLike,PDO::PARAM_STR);
        $st->bindValue(':q3',$qLike,PDO::PARAM_STR);
    ...
    public function countSubjectLoads($q){
        ...
        $sql = "SELECT COUNT(*) c
                ...
                WHERE t.full_name LIKE :q1 OR s.name LIKE :q2 OR sec.name LIKE :q3";
        $st = $this->pdo->prepare($sql);
        $st->execute([':q1'=>$qLike, ':q2'=>$qLike, ':q3'=>$qLike]);
 ...
 public function updateSubjectLoad($id, $teacher_id, $subject_id, $section_id, $school_year, $semester) {
    $sql = "UPDATE subject_loads SET 
    ...
    $stmt->bindParam(':sy', $school_year, PDO::PARAM_STR);
    $stmt->bindParam(':sem', $semester, PDO::PARAM_STR);
    ...
    public function getStudentGrades($student_id){
        $sql = "SELECT s.name AS subject_name, s.code AS subject_code, sec.name AS section_name,
                       e.school_year, e.semester,
                       g.q1, g.q2, g.q3, g.q4, g.grade
                FROM enrollments e
                JOIN subject_loads sl ON e.subject_load_id = sl.id
                JOIN subjects s ON sl.subject_id = s.id
                JOIN sections sec ON sl.section_id = sec.id
    ...
    public function upsertGrade($enrollment_id,$grade,$q1=null,$q2=null,$q3=null,$q4=null){
        $sel = $this->pdo->prepare("SELECT id FROM grades WHERE enrollment_id=:eid");
        $sel->execute([':eid'=>$enrollment_id]);
        if($sel->fetch()){
            $u = $this->pdo->prepare("UPDATE grades SET grade=:g, q1=:q1, q2=:q2, q3=:q3, q4=:q4 WHERE enrollment_id=:eid");
            return $u->execute([':g'=>$grade, ':q1'=>$q1, ':q2'=>$q2, ':q3'=>$q3, ':q4'=>$q4, ':eid'=>$enrollment_id]);
        }else{
            $i = $this->pdo->prepare("INSERT INTO grades(enrollment_id,grade,q1,q2,q3,q4) VALUES(:eid,:g,:q1,:q2,:q3,:q4)");
            return $i->execute([':eid'=>$enrollment_id, ':g'=>$grade, ':q1'=>$q1, ':q2'=>$q2, ':q3'=>$q3, ':q4'=>$q4]);
        }
    }

    public function getGradesForLoad($load_id){
        $sql = "SELECT g.enrollment_id, g.q1, g.q2, g.q3, g.q4, g.grade
                FROM grades g
    ...
    public function markAttendance($student_id,$subject_load_id,$date,$status){
        ...
        if($sel->fetch()){
            $u = $this->pdo->prepare("UPDATE attendance SET status=:st, recorded_at=CURRENT_TIMESTAMP WHERE student_id=:sid AND subject_load_id=:lid AND date=:d");
            return $u->execute([':st'=>$status, ':sid'=>$student_id, ':lid'=>$subject_load_id, ':d'=>$date]);
        }else{
            $i = $this->pdo->prepare("INSERT INTO attendance(student_id,subject_load_id,date,status,recorded_at) VALUES(:sid,:lid,:d,:st,CURRENT_TIMESTAMP)");
